...
class SingleLayerTranscoder(nn.Module):
    ...
    def __getattr__(self, name):
        ...
        if name == "W_enc" and self.lazy_encoder and self.transcoder_path is not None:
            with safe_open(
                self.transcoder_path, framework="pt", device=safetensors_mmap_device(self.device)
            ) as f:
                return f.get_tensor("W_enc").to(device=self.device, dtype=self.dtype)
        elif name == "W_dec" and self.lazy_decoder and self.transcoder_path is not None:
            with safe_open(
                self.transcoder_path, framework="pt", device=safetensors_mmap_device(self.device)
            ) as f:
                return f.get_tensor("W_dec").to(device=self.device, dtype=self.dtype)
    ...
    def _get_decoder_vectors(self, feat_ids=None):
        ...
        with safe_open(
            ...
            return f.get_slice("W_dec")[to_read].to(device=self.device, dtype=self.dtype)
...
def load_relu_transcoder(
    ...
    param_dict = {}
    with safe_open(path, framework="pt", device=safetensors_mmap_device(device)) as f:
        for k in f.keys():
            if lazy_encoder and k == "W_enc":
                continue
            if lazy_decoder and k == "W_dec":
                continue
            param_dict[k] = f.get_tensor(k).to(device=device)
