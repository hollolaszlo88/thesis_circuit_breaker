...
try:
    ...
    msg = str(exc)
    if "BACKENDS_MAPPING" in msg or "Offending backend" in msg:
        raise RuntimeError(
            "TransformerLens failed to import Hugging Face Transformers (lazy backend metadata error). "
            "This usually means a broken or mixed Transformers install, or a Jupyter kernel that needs "
            "a restart after upgrading packages.\n\n"
            "Try: restart the kernel, then reinstall a single Transformers build, e.g.\n"
            '  pip install --force-reinstall "transformers>=4.56.0,<=4.57.3"\n\n'
            "Or avoid TransformerLens entirely by using backend='nnsight' in "
...
from circuit_tracer.utils import configure_mps_memory_if_needed, get_default_device
...
class TransformerLensReplacementModel(HookedTransformer):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        configure_mps_memory_if_needed(device)
