...
from circuit_tracer.utils import get_default_device, safetensors_mmap_device
...
class CrossLayerTranscoder(torch.nn.Module):
    ...
    def _get_encoder_weights(self, layer_id=None):
        ...
        if layer_id is not None:
            ...
            with safe_open(enc_file, framework="pt", device=safetensors_mmap_device(self.device)) as f:
                return f.get_tensor(f"W_enc_{layer_id}").to(device=self.device, dtype=self.dtype)
        ...
        for i in range(self.n_layers):
            ...
            with safe_open(enc_file, framework="pt", device=safetensors_mmap_device(self.device)) as f:
                W_enc[i] = f.get_tensor(f"W_enc_{i}").to(device=self.device, dtype=self.dtype)
        return W_enc
    ...
    def _get_decoder_vectors(self, layer_id, feat_ids=None):
        ...
        if isinstance(to_read, torch.Tensor):
            to_read = to_read.cpu()
        with safe_open(path, framework="pt", device=safetensors_mmap_device(self.device)) as f:
            return f.get_slice(f"W_dec_{layer_id}")[to_read].to(device=self.device, dtype=self.dtype)
...
def _load_state_dict(
    ...
    with safe_open(
        os.path.join(clt_path, dec_file), framework="pt", device=safetensors_mmap_device(device)
    ) as f:
    ...
    for i in range(n_layers):
        ...
        with safe_open(
            os.path.join(clt_path, enc_file), framework="pt", device=safetensors_mmap_device(device)
        ) as f:
            b_dec[i] = f.get_tensor(f"b_dec_{i}").to(device=device, dtype=dtype)
            b_enc[i] = f.get_tensor(f"b_enc_{i}").to(device=device, dtype=dtype)

            # Only load W_enc if not lazy
            if not lazy_encoder:
                W_enc[i] = f.get_tensor(f"W_enc_{i}").to(device=device, dtype=dtype)

            if has_threshold:
                threshold = f.get_tensor(f"threshold_{i}").to(device=device, dtype=dtype)
        ...
        if not lazy_decoder:
            ...
            with safe_open(dec_file, framework="pt", device=safetensors_mmap_device(device)) as f:
                state_dict[f"W_dec.{i}"] = f.get_tensor(f"W_dec_{i}").to(device=device, dtype=dtype)
